...
type SeedBlueprint = Omit<SDLProject, '_id' | 'owner_id' | 'owner_name' | 'owner_avatar' | 'progress' | 'status' | 'featured' | 'trending' | 'views' | 'created_at'>;

type SeedVariant = {
  suffix: string;
  status: SDLProject['status'];
  progress: number;
  featured: boolean;
  trending: boolean;
  viewsBase: number;
...
const PROJECT_BLUEPRINTS: SeedBlueprint[] = [
  {
    title: 'Netflix Streaming Engine',
    project_type: 'system_replica',
    problem_statement: 'Deconstruct and rebuild the core video streaming pipeline with adaptive bitrate, CDN routing, and recommendation layers.',
  ...
  },
  {
    ...
    problem_statement: 'Build a collaborative filtering + content-based recommendation engine for an e-commerce platform with live ranking updates.',
  ...
  {
    title: 'Figma Collaboration Suite',
    project_type: 'system_replica',
    problem_statement: 'Design a live collaborative canvas with cursors, comments, and version history for distributed teams.',
    architecture_focus: 'Realtime Collaboration + OT',
    skills_required: ['TypeScript', 'WebSockets', 'Redis', 'Canvas API'],
    team_size: 4,
    timeline: '6 weeks',
    roles_needed: ['frontend', 'backend', 'ui_ux'],
    tags: ['Full Stack', 'System Design'],
    github_link: undefined,
  },
  {
    title: 'DevOps Release Orchestrator',
    project_type: 'original_build',
    problem_statement: 'Create a release coordination tool for approvals, canary rollouts, and audit trails across environments.',
    architecture_focus: 'Workflow Engine + Automation',
    skills_required: ['Python', 'Docker', 'Kubernetes', 'PostgreSQL'],
    team_size: 3,
    timeline: '5 weeks',
  ...
  {
    title: 'Fintech Fraud Radar',
    project_type: 'original_build',
    problem_statement: 'Detect suspicious payment patterns in real time using streaming features and anomaly scoring.',
    architecture_focus: 'Streaming Risk Scoring',
    skills_required: ['Python', 'Kafka', 'Redis', 'FastAPI', 'MLflow'],
    team_size: 3,
    timeline: '6 weeks',
    roles_needed: ['ai', 'backend'],
    tags: ['AI', 'Backend'],
  ...
  },
  {
    ...
    project_type: 'original_build',
    problem_statement: 'Build a study assistant that personalizes lessons, reminders, and quiz generation based on learner behavior.',
    architecture_focus: 'Personalization Engine + LLM',
    skills_required: ['Python', 'OpenAI API', 'React', 'PostgreSQL'],
    team_size: 2,
    timeline: '4 weeks',
    roles_needed: ['ai', 'frontend'],
    tags: ['AI', 'Beginner Friendly'],
    github_link: undefined,
  },
  {
    title: 'Health Appointment Mesh',
    project_type: 'system_replica',
    problem_statement: 'Create a healthcare scheduling mesh with notifications, availability sync, and secure patient routing.',
    architecture_focus: 'Scheduling + Secure APIs',
    skills_required: ['Node.js', 'Redis', 'PostgreSQL', 'React', 'Docker'],
    team_size: 4,
    timeline: '5 weeks',
    roles_needed: ['backend', 'frontend', 'devops'],
  ...
  },
  {
    ...
    project_type: 'original_build',
    problem_statement: 'Give creators a dashboard for audience trends, revenue insights, and growth experiments.',
    architecture_focus: 'Analytics Platform + BI',
    skills_required: ['React', 'TypeScript', 'PostgreSQL', 'Supabase'],
    team_size: 2,
    timeline: '4 weeks',
...
const PROJECT_VARIANTS: SeedVariant[] = [
  { suffix: 'MVP', status: 'open', progress: 0, featured: true, trending: true, viewsBase: 140, timeline: '2 weeks' },
  { suffix: 'Prototype', status: 'open', progress: 0, featured: true, trending: false, viewsBase: 165, timeline: '3 weeks' },
  { suffix: 'Sprint', status: 'in_progress', progress: 35, featured: false, trending: true, viewsBase: 190, timeline: '4 weeks' },
  { suffix: 'Scale', status: 'in_progress', progress: 58, featured: false, trending: true, viewsBase: 225, timeline: '5 weeks' },
  { suffix: 'Launch', status: 'completed', progress: 100, featured: false, trending: false, viewsBase: 270, timeline: '6 weeks' },
  { suffix: 'Plus', status: 'open', progress: 12, featured: false, trending: false, viewsBase: 305, timeline: '3 weeks' },
  { suffix: 'Pro', status: 'in_progress', progress: 47, featured: false, trending: true, viewsBase: 340, timeline: '4 weeks' },
  { suffix: 'Elite', status: 'completed', progress: 100, featured: true, trending: false, viewsBase: 390, timeline: '5 weeks' },
  { suffix: 'Studio', status: 'open', progress: 18, featured: false, trending: false, viewsBase: 430, timeline: '6 weeks' },
];

const SEED_PROJECTS: SDLProject[] = PROJECT_BLUEPRINTS.flatMap((blueprint, blueprintIndex) =>
  PROJECT_VARIANTS.map((variant, variantIndex) => ({
    _id: `seed-${blueprintIndex + 1}-${variantIndex + 1}`,
    owner_id: 'system',
    owner_name: 'Studlyf Lab',
    owner_avatar: undefined,
    title: `${blueprint.title} ${variant.suffix}`,
    project_type: blueprint.project_type,
    problem_statement: `${blueprint.problem_statement} Focus on the ${variant.suffix.toLowerCase()} milestone and ship it in ${variant.timeline}.`,
    architecture_focus: blueprint.architecture_focus,
    skills_required: blueprint.skills_required,
    team_size: blueprint.team_size,
    timeline: variant.timeline,
    roles_needed: blueprint.roles_needed,
    tags: blueprint.tags,
    github_link: blueprint.github_link,
    progress: variant.progress,
    status: variant.status,
    featured: variant.featured && blueprintIndex < 4,
    trending: variant.trending || blueprintIndex % 3 === 0,
    views: variant.viewsBase + blueprintIndex * 17 + variantIndex * 11,
    created_at: new Date(Date.now() - (blueprintIndex * 9 + variantIndex) * 86400000).toISOString(),
  }))
);
